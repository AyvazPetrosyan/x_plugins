...
class ZilProductImport extends Plugin
{
    ...
    private function createAdditionalAttributes()
    {
        ...
        if (!$crudService->get('s_articles_attributes', 'addresses')) {
            $crudService->update('s_articles_attributes', 'addresses', TypeMapping::TYPE_STRING, [
                    'label' => $snippetsNamespace->get('addressesLabel', 'addresses'),
                    'supportText' => $snippetsNamespace->get('addressesSupportText', 'addresses'),
                    'helpText' => $snippetsNamespace->get('addressesHelpText', 'addresses'),
                    'translatable' => true,
        ...
        if (!$crudService->get('s_articles_attributes', 'zil_is_new')) {
            $crudService->update('s_articles_attributes', 'zil_is_new', TypeMapping::TYPE_BOOLEAN, [
                    'label' => $snippetsNamespace->get('isNewLabel', 'Is new'),
                    'supportText' => $snippetsNamespace->get('isNewSupportText', 'Is new'),
                    'helpText' => $snippetsNamespace->get('isNewHelpText', 'Is new'),
                    'translatable' => true,
                    'displayInBackend' => false,
                    'position' => 0,
                    'custom' => false
    ...
    private function removeAdditionalAttributes()
    {
        ...
        if ($crudService->get('s_articles_attributes', 'addresses')) {
            $crudService->delete('s_articles_attributes', 'addresses');
        }
        ...
        if ($crudService->get('s_articles_attributes', 'zil_is_new')) {
            $crudService->delete('s_articles_attributes', 'zil_is_new');
        }
