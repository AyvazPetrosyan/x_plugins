...
class ProductImport
{
    ...
    public function removeProducts($productsId, $removeAll = false, $removeCategories = false, $removePrice = false)
    {
        ...
        Shopware()->Db()->query("DELETE FROM media_package");
    ...
    private function createProduct($newProductList)
    {
        ...
        try {
            foreach ($newProductList as $productKey => $productInfo) {
                ...
                $productInfo['parentNumber'] = Shopware()->Db()->fetchOne('SELECT product_number FROM `products` WHERE parent_number=? AND is_main=1', [$productInfo['parentNumber']]);
        ...
        if (!empty($changedProductList)) {
            try {
                foreach ($changedProductList as $changedProduct => $changedProductInfo) {
                    ...
                    $changedProductInfo['active'] = 1;
                    $changedProductInfo['parentNumber'] = Shopware()->Db()->fetchOne('SELECT product_number FROM `products` WHERE parent_number=? AND is_main=1', [$changedProductList['parentNumber']]);
    ...
    private function importArticleAttr($productInfo)
    {
        ...
        $productAttr->setZilIsNew($productInfo['isNew']);
        $productAttr->setAddresses($productInfo['address']);
